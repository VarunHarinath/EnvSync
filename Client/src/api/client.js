...
export const MOCK_DB = {
  ...
  environments: [
    ...
    },
    {
      ...
      project_id: "proj_1",
      name: "Staging",
      created_at: "2023-10-15T10:06:00Z",
    },
    {
      id: "env_3",
      project_id: "proj_1",
    ...
    },
    {
      ...
      project_id: "proj_2",
      name: "Production",
      created_at: "2023-11-01T09:20:00Z",
    },
  ],
  secrets: [
    {
      ...
      project_id: "proj_1",
      name: "DATABASE_URL",
      value: "postgres://user:pass@db.prod:5432/db",
      updated_at: "2023-10-20T14:30:00Z",
    },
    {
      id: "sec_2",
  ...
  api_keys: [
    ...
    },
    {
      id: "key_3",
      project_id: "proj_1",
      environment_id: "env_3",
      key_prefix: "sk_dev_9012",
...
function handleMockRequest(endpoint, config) {
  ...
  // --- PROJECTS ---
  if (endpoint === "/project/getProjects" && method === "GET")
  ...
  if (
    endpoint.match(/^\/project\/getProjectById\/[^/]+$/) &&
    method === "GET"
  ) {
  ...
  if (
    endpoint.match(/^\/project\/deleteproject\/[^/]+$/) &&
    method === "DELETE"
  ) {
    const id = endpoint.split("/").pop();
  ...
  if (
    endpoint.match(/^\/project\/updateProjectById\/[^/]+$/) &&
    method === "PATCH"
  ) {
    const id = endpoint.split("/").pop();
    const p = MOCK_DB.projects.find((x) => x.id === id);
  ...
  // --- ENVIRONMENTS ---
  if (
    endpoint.match(/^\/environment\/getEnvironmentById\/[^/]+$/) &&
    method === "GET"
  ) {
  ...
  if (
    endpoint.match(/^\/environment\/deleteEnvironmentById\/[^/]+$/) &&
    method === "DELETE"
  ) {
    const id = endpoint.split("/").pop();
  ...
  if (
    endpoint.match(/^\/environment\/updateEnvironmentById\/[^/]+$/) &&
    method === "PATCH"
  ) {
    const id = endpoint.split("/").pop();
  ...
  // --- SECRETS ---
  if (
    endpoint.match(/^\/secret\/getSecretsByProjectId\/[^/]+$/) &&
    method === "GET"
  ) {
    const projectId = endpoint.split("/").pop();
    ...
    return {
      ...
      getSecretValueBySecretId: secrets.map((s) => ({
        secret_id: s.id,
        value: s.value,
      })),
    };
  ...
  if (
    endpoint.match(/^\/secret\/deleteSecretBySecretId\/[^/]+$/) &&
    method === "DELETE"
  ) {
    const id = endpoint.split("/").pop();
  ...
  if (
    endpoint.match(/^\/secret\/updateSecretsBySecretId\/[^/]+$/) &&
    method === "PATCH"
  ) {
    const id = endpoint.split("/").pop();
